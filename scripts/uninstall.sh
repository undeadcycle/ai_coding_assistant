...
remove_conda_env() {
  ...
  if conda env list | grep -q "^$env_name\s"; then
    ...
    log "Environment $env_name not found."
  fi
}
...
remove_jupyter_kernel() {
  ...
  if jupyter kernelspec list | grep -q "$kernel_name"; then
    ...
    log "Jupyter kernel $kernel_name not found."
  fi
}
...
remove_directory() {
  ...
  if [ -d "$dir_path" ]; then
    ...
    log "Directory $dir_path not found."
  fi
}
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  find "${ROOT_PATH}" -name '__pycache__' -type dxec rm -rf {} +
